...
namespace Dwapi.Prep.SharedKernel.Tests.TestData.TestData
{
    public class TestDataFactory
    {
        ...
        public static List<Facility> TestFacilityWithPatients(int count = 2, int childcount = 3)
        {
            ...
            foreach (var facility in facilities)
            {
                n++;
                facility.SiteCode = n;
                facility.PrepPatients = Builder<PatientPrep>.CreateListOfSize(childcount)
                    .All()
                    .With(x => x.FacilityId == facility.Id)
                    .Build()
                    .ToList();

                facility.PrepAdverseEvents = Builder<PrepAdverseEvent>.CreateListOfSize(childcount)
                    .All()
                    .With(x => x.FacilityId == facility.Id)
                    .Build()
                    .ToList();
        ...
        public static List<PatientPrep> TestClients(int siteCode, Guid facilityId, int count = 5)
        {
            var patientIndices = Builder<PatientPrep>.CreateListOfSize(count)
